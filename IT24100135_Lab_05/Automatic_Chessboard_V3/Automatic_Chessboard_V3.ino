...
void motor(byte direction, int speed, float distance) {
    ...
    if (distance == calibrate_speed) step_number = 4;
        ...
        step_number = distance * SQUARE_SIZE * 1.44; // Diagonal
    } else {
        step_number = distance * SQUARE_SIZE;
    }

    // Set motor direction
    digitalWrite(MOTOR_WHITE_DIR, (direction == R_L || direction == T_B || direction == RL_TB));
    digitalWrite(MOTOR_BLACK_DIR, (direction == B_T || direction == R_L || direction == RL_BT));
    ...
    // Active the motors with error handling
...
void lcd_display() {
  ...
  switch (sequence) {
    case start_up:
      ...
      Serial.println("Starting Automatic Chessboard...");
      sequence = start;
      delay(4000);
    ...
    case start:
      ...
      Serial.println("Waiting for player selection (HvsH or HvsC).");
      break;
    ...
    case player_white:
      ...
      Serial.print("White's turn. Time: ");
      Serial.print(minute);
      Serial.print(":");
    ...
    case player_black:
      ...
      lcd.setCursor(0, 1);
      lcd.print("     " + String(minute) + " : " + String(second) + "     ");
      Serial.print("Black's turn. Time: ");
      Serial.print(minute);
      Serial.print(":");
      Serial.println(second);
      break;
